 ...
 * @author ATH
 */
public class PrimaireEvaluationMatiereTypeController {

    private final Connection connection;
    private PreparedStatement preparedStatement;
    private ResultSet resultSet;

    public PrimaireEvaluationMatiereTypeController(Connection connection) {
    ...
    private void add(Long id_eleve, Long id_classe, Long id_matiere_type, Long id_semestre) {
        try {
            ...
            Logger.getLogger(PrimaireEvaluationMatiereTypeController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void update(Long id_eleve, Long id_classe, Long id_matiere_type, Long id_semestre) {
        ...
        try {
            String req;
            req = "SELECT "
                    + "SUM(evaluation_matieres.composition) AS somme_composition, "
                    ...
                    + "WHERE evaluation_matieres.id_eleve = ? AND matiere_classes.id_classe = ? AND matieres.id_matiere_type = ? AND evaluation_matieres.id_semestre = ? AND evaluation_matieres.composition > '-1.00' AND evaluation_matieres.composition IS NOT NULL ";
            preparedStatement = connection.prepareStatement(req);
            preparedStatement.setLong(1, id_eleve);
            preparedStatement.setLong(2, id_classe);
            preparedStatement.setLong(3, id_matiere_type);
            ...
            if (resultSet.next()) {
                double numerateur = resultSet.getDouble("somme_composition");
                ...
                preparedStatement = connection.prepareStatement(req);
                preparedStatement.setDouble(1, numerateur * 2 / denomunateur);
            ...
            req = "SELECT DISTINCT moyenne FROM evaluation_matiere_types WHERE id_classe = ? AND id_matiere_type = ? AND id_semestre = ? AND moyenne IS NOT NULL GROUP BY moyenne ORDER BY moyenne DESC ";
            preparedStatement = connection.prepareStatement(req);
